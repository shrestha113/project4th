...
// Initialize variables for assignment data
$assignment_id = "";
$course = "";
$semester = "";
$title = "";
...
if (isset($_GET['editid'])) {
    ...
    $assignment_id = $_GET['editid'];
    
    // Query to fetch assignment details based on assignment ID
    $query_assignment = mysqli_query($con, "SELECT * FROM assignment WHERE id='$assignment_id'");
    if ($query_assignment && mysqli_num_rows($query_assignment) > 0) {
        $row_assignment = mysqli_fetch_assoc($query_assignment);
        $course = $row_assignment['Course'];
        $semester = $row_assignment['Semester'];
        $title = $row_assignment['Title'];
        $upload = $row_assignment['Upload'];
    }
}

// Check if the form is submitted
if (isset($_POST['submit'])) {// Assuming you have a hidden input field for assignment ID in your form
    $course = $_POST['course'];
    ...
    if ($_FILES['file']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileType, $allowTypes)) {
            ...
            if (move_uploaded_file($_FILES["file"]["tmp_name"], $targetFilePath)) {
                ...
                $query = mysqli_prepare($con, "UPDATE assignment SET Course=?, Semester=?, Title=?, Upload=? WHERE id=?");
                mysqli_stmt_bind_param($query, "ssssi", $course, $semester, $title, $fileName, $assignment_id);
                ...
                if ($result) {
                    ...
                    echo "<script>alert('Failed to update assignment');</script>";
                }
            } else {
        ...
        // If no file is uploaded, update only other data without changing the upload field
        $query = mysqli_prepare($con, "UPDATE assignment SET Course=?, Semester=?, Title=? WHERE id=?");
        mysqli_stmt_bind_param($query, "sssi", $course, $semester, $title, $assignment_id);
        $result = mysqli_stmt_execute($query);
            ...
            <div class="container-xl">
                <div class="table-responsive">
                    <div class="table-wrapper">
                     ...
                     <?php $eid = $_GET['editid'];
                        $ret = mysqli_query($con, "SELECT * FROM assignment WHERE id='$eid'");
